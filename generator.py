...
AV_KEY = os.environ.get("ALPHA_VANTAGE_KEY", "demo")   # get free key at alphavantage.co

# Asset pool — rotated across the 5 daily slots
ASSET_POOL = [
    # Crypto (CoinGecko)
    ...
    # Commodities / indices via Alpha Vantage
    {"asset": "XAU/USD",  "source": "av_stock",  "symbol": "GLD"},   # Gold ETF proxy
    {"asset": "USO/USD",  "source": "av_stock",  "symbol": "USO"},   # Oil ETF proxy
    {"asset": "SPX500",   "source": "av_stock",  "symbol": "SPY"},   # S&P 500 ETF proxy
    {"asset": "NAS100",   "source": "av_stock",  "symbol": "QQQ"},   # Nasdaq ETF proxy
]
...
def _generate_signal_from_price(asset_meta: dict, price: float, prices_history: list[float]) -> dict:
    """Apply RSI + EMA crossover to decide direction, then calculate TP/SL."""
...
class SignalGenerator:
    ...
    def _pick_asset(self, slot_index: int) -> dict:
        ...
        # Deterministic by slot so same asset isn't repeated
    ...
    async def _from_coingecko(self, session: aiohttp.ClientSession, meta: dict) -> dict:
        # Get 30-day daily prices (free, no key)
        url = (
    ...
    # ── Fallback (static last-known price if APIs fail) ───────────────────────
